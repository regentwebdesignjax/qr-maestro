...
import { User, Phone, Mail, Globe, Linkedin, Instagram } from 'lucide-react';
    ...
    <div className="flex justify-center">
      ...
      <div className="w-[280px] rounded-[2rem] border-[6px] border-gray-800 shadow-2xl overflow-hidden bg-white">
        ...
        <div className="overflow-y-auto max-h-[520px] bg-gray-50">
          ...
          {/* Identity block — padded to clear the overlapping headshot */}
          <div className="pt-12 px-4 pb-2 text-center">
            ...
            {data.pronouns && <p className="text-xs text-gray-400">{data.pronouns}</p>}
            {data.company && (
          ...
          <div className="px-4 pb-4 space-y-1.5">
            ...
            {data.linkedin && (
              <div className="flex items-center gap-2 p-2 bg-white rounded-lg border border-gray-100">
                ...
                <span className="text-xs text-gray-700 truncate">{data.linkedin}</span>
              </div>
            )}
            {data.instagram && (
              <div className="flex items-center gap-2 p-2 bg-white rounded-lg border border-gray-100">
                <Instagram className="w-3.5 h-3.5 text-gray-400 shrink-0" />
                <span className="text-xs text-gray-700 truncate">{data.instagram}</span>
              </div>
            )}
          </div>
        </div>
