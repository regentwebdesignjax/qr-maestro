...
export default function FoldersSidebar({ activeFolder, onFolderChange }) {
  ...
  const commitAdd = () => {
    ...
    const newFolder = { id: `folder_${Date.now()}`, name: newFolderName.trim(), locked: false };
                ...
                {!folder.locked && (
                  <div className="hidden group-hover:flex items-center gap-0.5">
                    <button
                      ...
                      className="p-0.5 rounded hover:text-blue-600"
                    >
                      <Pencil className="w-3 h-3" />
                    </button>
                    <button
                      onClick={(e) => { e.stopPropagation(); deleteFolder(folder.id); }}
                      className="p-0.5 rounded hover:text-red-600"
                    >
