...
import { Phone, Mail, Globe, Linkedin, Instagram, UserPlus, ArrowRight } from 'lucide-react';
...
export default function BusinessCardDisplay({ data }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      {/* Main content — padded to clear headshot */}
      <div className="flex-1 pt-20 px-4 pb-8 max-w-md mx-auto w-full">
        ...
        <div className="text-center mb-6">
          ...
          {data.pronouns && <p className="text-sm text-gray-400">{data.pronouns}</p>}
        ...
        <div className="space-y-2">
          ...
          {data.linkedin && (
            <a href={data.linkedin.startsWith('http') ? data.linkedin : `https://${data.linkedin}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3.5 bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              <Linkedin className="w-4 h-4 text-gray-400 shrink-0" />
              <span className="text-sm text-gray-700 truncate">{data.linkedin}</span>
            </a>
          )}
          {data.instagram && (
            <a href={data.instagram.startsWith('http') ? data.instagram : `https://instagram.com/${data.instagram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3.5 bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              <Instagram className="w-4 h-4 text-gray-400 shrink-0" />
              <span className="text-sm text-gray-700 truncate">{data.instagram}</span>
            </a>
          )}
        </div>
      </div>
