...
export default function MyQRCodes() {
  ...
  const isPro = user ? (user.role === 'admin' || (['black_belt', 'grand_master'].includes(user.subscription_tier) && user.subscription_status === 'active')) : false;
  const subActive = user ? (user.role === 'admin' || !['black_belt', 'grand_master'].includes(user.subscription_tier) || user.subscription_status === 'active') : false;
